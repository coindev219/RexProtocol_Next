...
*/
    constructor(address initialOwner, address airnodeRrpAddress) Ownable(initialOwner) RrpRequesterV0(airnodeRrpAddress) {}
    ...
    // pair - fixed 2 size array - first item is taker token
    ///address[2] memory pair = [taker_token, maker_token];

    // participants - nested array of addresses
    // trade_amounts - nested array of uint256

    /// fixed 3 size array airnode, sponsor, sponsor wallet 
    ...
    function ProcessTrade(
        ...
        address[] memory makers = participants[0];
        address[] memory takers = participants[1];
        uint256[] memory taker_amounts = trade_amounts[0];
        uint256[] memory maker_amounts = trade_amounts[1];
        ...
        bytes32 orderId = makeRequest(airnode_details[0], endpointId, airnode_details[1], airnode_details[2], parameters);
    ...
    function TransferBalances(
        ...
        for (uint256 i = 0; i < takers.length; i++) {
            ...
            token_deposit_info[takers[i]][maker_token] = token_deposit_info[
                takers[i]
            ][maker_token].add(maker_amounts[i]).sub(calculateFee(maker_amounts[i], fee));
        }
        for (uint256 i = 0; i < makers.length; i++) {
            ...
            token_deposit_info[makers[i]][taker_token] = token_deposit_info[
                makers[i]
            ][taker_token].add(taker_amounts[i]).sub(calculateFee(taker_amounts[i], fee));
        }
        for (uint256 i = 0; i < makers.length; i++) {
    ...
    function setFeePercent(uint256 _fee) external onlyOwner {
        require(_fee <= 10**4, "Fee exceeded max limitation");
        fee = _fee;
    }
